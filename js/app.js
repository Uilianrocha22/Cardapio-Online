...
let valorEntrega = 7.5;

let celularEmpresa = `5511983331546`;

// evento para carregar o documento
...
cardapio.metodos = {
  ...
  obterItensCardapio: (categoria = "burgers", vermais = false) => {
    ...
    let itensCardapio = document.getElementById(`itensCardapio`);
    ...
    filtro.forEach((elemento, index) => {
      let temple = cardapio.templates.item
        .replace(/\${img}/g, elemento.img)
        .replace(/\${name}/g, elemento.name)
        .replace(/\${preço}/g, elemento.price.toFixed(2).replace(".", ","))
        .replace(/\${id}/g, elemento.id);

      // botão ver mais foi clicado (12 itens)
      if (vermais && index >= 8 && index < 12) {
        // itensCardapio.append(temple);
        itensCardapio.innerHTML += temple;
      }
    ...
    // remover classe active dos itens
    $(".container-menu a").removeClass(`active`);

    // adicionando classe active nos itens
  ...
  diminuirQuantidade: (id) => {
    let qntdAtual = Number($("#qntd-" + id).text());
  ...
  adicionarCarrinho: (id) => {
    ...
    if (qntdAtual > 0) {
      ...
      // obter o item
      let item = $.grep(filtro, (elemento, index) => {
      ...
      if (item.length > 0) {
        ...
        let existeItem = $.grep(meuCarrinho, (elemento, index) => {
          return elemento.id == id;
        });

        // caso ja exista o item no carrinho, só altera a quantidade
        if (existeItem.length > 0) {
          let arrayIndex = meuCarrinho.findIndex((array) => array.id == id);

          meuCarrinho[arrayIndex].qntd += qntdAtual;
        }
  ...
  // atualiza o badge total dos botões "meu carrinho"
  atualizarBadgeTotal: () => {
    let total = 0;

    $.each(meuCarrinho, (index, elemento) => {
      total += elemento.qntd;
    });
  ...
  carregarEtapa: (etapa) => {
    if (etapa == 1) {
      ...
      $(".etapas .etapa").removeClass(`active`);
  ...
  // botão de voltar as etapas do pedido
  voltarEtapa: () => {
  ...
  carregarCarrinho: () => {
    ...
    if (meuCarrinho.length > 0) {
      ...
      $.each(meuCarrinho, (indice, elemento) => {
        let temple = cardapio.templates.itemCarrinho
          .replace(/\${img}/g, elemento.img)
          .replace(/\${name}/g, elemento.name)
          .replace(/\${preço}/g, elemento.price.toFixed(2).replace(".", ","))
          .replace(/\${id}/g, elemento.id)
          .replace(/\${qntd}/g, elemento.qntd);

        $("#itensCarrinho").append(temple);

        // quando carregar o último item do carrinho
        if (indice + 1 == meuCarrinho.length) {
          cardapio.metodos.carregarValores();
        }
      });
    } else {
      $("#itensCarrinho").html(
        '<p class="carrinhoVazio"><i class="fa fa-shopping-bag"></i> Seu carrinho está vazio.</p>'
      );
  ...
  diminuirQuantidadeCarrinho: (id) => {
    let qntdAtualCarrinho = Number($("#qntd-carrinho-" + id).text());

    if (qntdAtualCarrinho > 1) {
      $("#qntd-carrinho-" + id).text(qntdAtualCarrinho - 1);
      cardapio.metodos.atualizarCarrinho(id, qntdAtualCarrinho - 1);
    } else {
      cardapio.metodos.removerItemCarrinho(id);
    }
  },

  // almentar quantidade do item no carrinho
  aumentarQuantidadeCarrinho: (id) => {
    let qntdAtualCarrinho = Number($("#qntd-carrinho-" + id).text());

    if (qntdAtualCarrinho >= 0) {
      $("#qntd-carrinho-" + id).text(qntdAtualCarrinho + 1);
      cardapio.metodos.atualizarCarrinho(id, qntdAtualCarrinho + 1);
    }
  },
  ...
  removerItemCarrinho: (id) => {
    meuCarrinho = $.grep(meuCarrinho, (elemento, index) => {
      return elemento.id != id;
    });
    cardapio.metodos.carregarCarrinho();
  ...
  atualizarCarrinho: (id, qntd) => {
    let obj = meuCarrinho.findIndex((obj) => obj.id == id);
    meuCarrinho[obj].qntd = qntd;

    // atualiza o botão carrinho com a quantidade atualizada
    cardapio.metodos.atualizarBadgeTotal();

    // atualiza os valores em reais do total do carrinho
    cardapio.metodos.carregarValores();
  },
  ...
  carregarValores: () => {
    ...
    $.each(meuCarrinho, (index, elemento) => {
      valorCarrinho += Number(elemento.price * elemento.qntd);

      // forEach para localizar o ultimo item do carrinho e alterar os valores
      if (index + 1 == meuCarrinho.length) {
        $("#lblSubtotal").text(
        ...
        $("#lblValorTotal").text(
          `R$ ${(valorCarrinho + valorEntrega).toFixed(2).replace(".", ",")}`
        );
      }
    });
  },
  ...
  buscarCep: () => {
    ...
    if (cep != "") {
      ...
      if (validarCep.test(cep)) {
        ...
        cardapio.metodos.mensagens(`Formato do CEP inválido.`);
        $("#txtCEP").focus();
      }
    } else {
      cardapio.metodos.mensagens("informe o CEP, por favor.");
      $("#txtCEP").focus();
    }
  },
  ...
  resumoPedido: () => {
    ...
    if (endereco.length <= 0) {
      cardapio.metodos.mensagens(`Informe o endereço, por favor.`);
      $("#txtEndereco").focus();
      return;
    }
    if (bairro.length <= 0) {
      cardapio.metodos.mensagens(`Informe o bairro, por favor.`);
      $("#txtBairro").focus();
      return;
    }
    if (cidade.length <= 0) {
      cardapio.metodos.mensagens(`Informe a cidade, por favor.`);
      $("#txtCidade").focus();
      return;
    }

    if (numero.length <= 0) {
      cardapio.metodos.mensagens(`Informe o Número, por favor.`);
      $("#txtNumero").focus();
      return;
    }
    if (uf == "-1") {
      cardapio.metodos.mensagens(`Informe a UF, por favor.`);
      $("#txtUf").focus();
      return;
    }
  ...
  // carrega a ultima etapa de Resumo do pedido
  carregarResumo: () => {
    $(`#listaItensResumo`).html("");

    $.each(meuCarrinho, (indice, elemento) => {
      let temple = cardapio.templates.itemResumo
        .replace(/\${img}/g, elemento.img)
        .replace(/\${name}/g, elemento.name)
        .replace(/\${preço}/g, elemento.price.toFixed(2).replace(".", ","))
        .replace(/\${qntd}/g, elemento.qntd);

      $("#listaItensResumo").append(temple);
    });
  ...
  //finalizar pedido e atualizar o link do botão do whatsapp
  finalizarPedido: () => {
    if (meuCarrinho.length > 0 && meuEndereco != null) {
      ...
      \n\n*Total com entrega: RS ${(valorCarrinho + valorEntrega)
        .toFixed(2)
        .replace(".", ",")}*
      `;
      ...
      $.each(meuCarrinho, (index, elemento) => {
        itens += `*${elemento.qntd}x* ${
          elemento.name
        } .......R$ (${elemento.price.toFixed(2).replace(".", ",")})\n`;
  ...
  carregarBotaoReserva: () => {
    let texto = `Ola! gostaria de fazer uma *reserva*`;
  ...
  // carrega p whatsapp no clique do btn whatsapp
  carregarBotaoWhatsapp: () => {
    let URL = `https://wa.me/${celularEmpresa}?text=Seja bem vindo!`;
  ...
  // mensagens dinâmica
  mensagens: (texto, cor = "red") => {
    let idMensagem = Math.floor(Date.now() * Math.random().toString());

    let msg = `<div id="msg-${idMensagem}" class="animated fadeInDown toast ${cor}">${texto}</div>`;
...
cardapio.templates = {
  ...
  <div class="card card-item" id="\${id}">
     ...
     <span class="btn-menos" onclick="cardapio.metodos.diminuirQuantidade('\${id}')">
      ...
      <span class="btn btn-add" onclick="cardapio.metodos.adicionarCarrinho('\${id}')">
  ...
  itemCarrinho: `
    <div class="col-12 item-carrinho">
       ...
       <span class="add-numero-itens" id="qntd-carrinho-\${id}" onclick="cardapio.metodos.removerItemCarrinho()">\${qntd}</span>
  ...
  itemResumo: `
      <div class="col-12 item-carrinho resumo">
         <div class="img-produto-resumo">
            <img src="\${img}" alt="imagem hamburguer">
         </div>
         <div class="dados-produto">
